...
async function yts(query, page = '1') {
    ...
    // ⚡ BOLT OPTIMIZATION: Race YTS API mirrors concurrently using Promise.any
    // Eliminates sequential mirror request delays by returning the fastest valid mirror response.
    const mirrorPromises = YTS_API_MIRRORS.map(async (mirror) => {
